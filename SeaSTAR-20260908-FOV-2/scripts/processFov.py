...
def polygonArea(X):
	...
	return 0.5*(x[:-1]*y[1:] - x[1:]*y[:-1]).sum()

def isClosedPolygon(seg):
	# a centroid by the shoelace sum only means anything on a closed ring, and
	# matplotlib repeats the first vertex to close one.  three points is the
	# fewest that can enclose any area, so four with the repeat.
	return len(seg) >= 4 and np.allclose(seg[0], seg[-1])

def peakCenter(cont):
	"""Centroid of the innermost contour level that gives a usable closed ring.

	The obvious allsegs[-1][0] is the first segment of the innermost level,
	which on a broad flat top is often a short open fragment running along the
	edge of the plateau: its centroid lands on the rim of the spot rather than
	in the middle of it.  Take the largest closed ring instead, and step out a
	level when the innermost has none.

	Returns (centre, level), or (None, None) if no level closes."""
	for level in range(len(cont.allsegs) - 1, -1, -1):
		rings = [seg for seg in cont.allsegs[level] if isClosedPolygon(seg)]
		
		if not rings:
			continue
		
		best = max(rings, key=lambda seg: abs(polygonArea(seg)))
		
		# a zero-area ring doubles back on itself and would divide by zero:
		if abs(polygonArea(best)) > 0:
			return centerOfMass(best), level
...
for j in range(len(channels)):
	...
	com, level = peakCenter(cont)
	
	if com is not None:
		innermost = len(cont.allsegs) - 1
		note = "" if level == innermost else f"  (from level {level} of {innermost}, no closed ring further in)"
		print(f"Channel {channelNumbers[j]}: {com}{note}")
