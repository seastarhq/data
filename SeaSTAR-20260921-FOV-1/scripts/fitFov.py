...
# between grid points and are left out of the cell average.  it has to be wider
# than the stage's steady-state error, or settled samples are thrown away and
# cells come out empty: on 2026-09-21 the stage sat 0.0034 off the commanded
# position all scan, and at 0.002 only 283 of the 441 cells were filled:
defaultSettleTolerance = 0.002

# warn when this fraction of the grid ends up with no settled samples in it,
# which means the tolerance is tighter than the stage's own error:
emptyCellWarning = 0.05
...
parser.add_argument("--settle", type=float, default=defaultSettleTolerance, help=f"how far a sample may sit from the commanded position and still count as settled (default: {defaultSettleTolerance})")
...
settled = np.hypot(sx - tx, sy - ty) < args.settle

print(f"settled: {settled.sum()} of {len(settled)} samples, within {args.settle} of the commanded position")
...
empty = np.count_nonzero(numGrid == 0)

if empty > emptyCellWarning*numGrid.size:
	error = np.median(np.hypot(sx - tx, sy - ty))

	print(f"warning: {empty} of {numGrid.size} cells have no settled samples; the stage sat a median "
		f"{error:.4f} from the commanded position")

	# only the first case is the tolerance's fault: if the stage was inside it
	# on average then those cells are missing for some other reason, such as
	# points the scan never dwelt on.
	if error >= args.settle:
		print(f"         --settle {args.settle} is tighter than that, so settled samples are being thrown away")
	else:
		print(f"         that is inside --settle {args.settle}, so check the scan reached those points")
