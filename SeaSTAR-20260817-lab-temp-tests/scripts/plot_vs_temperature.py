...
def clean(df, keepdisturbed, keeplampoff, keepheateron):
    ...
    Heater-on samples go too: while the heater is driving the block the sensors
    lead the optics, so the signal is being read against a temperature the rest
    of the block has not reached yet. The cool-down afterwards covers the same
    range slowly enough for the two to keep up with each other.
    """
    if not keepheateron and "heater_on" in df.columns:
        heating = df["heater_on"].astype(bool)
        if heating.any():
            sys.stderr.write(f"leaving out {int(heating.sum())} heater-on samples "
                             "(-H keeps them)\n")
        df = df[~heating]
...
def plot_against_temperature(df, gain, outdir, datestamp, fullrange):
    ...
    fig.suptitle(f"SeaSTAR J3 channels, gain {gain}, against block temperature, "
                 f"{datestamp}", fontsize=12, color=TEXT_PRIMARY, x=0.01,
                 ha="left")
    fig.supylabel("signal (V)", fontsize=9, color=TEXT_SECONDARY)
    fig.tight_layout(rect=(0.01, 0, 1, 0.97))

    outfile = os.path.join(outdir,
                           f"channels_gain{gain}_vs_temperature_{datestamp}.png")
...
# out, and how the value underneath it is written.
...
def fit_row(channel, gain, fitted):
    slope, intercept, rsquared, n = fitted
    values = [channel, gain, slope, slope * 1e6, intercept, rsquared, n]
    return "  ".join(f"{value:{spec}}"
                     for (_, _, spec), value in zip(FIT_COLUMNS, values))


def write_fits(path, gain, fits, notes):
    """Write the coefficients table, keeping the rows for the other gains.

    One run of the script only sees one gain setting, so the rows already in
    the file for the gains it did not plot are read back and kept: that way a
    make of every gain leaves one table with all of them in it.
    """
    rows = []
    if os.path.exists(path):
        with open(path) as existing:
            for line in existing:
                fields = line.split()
                if len(fields) > 1 and not line.startswith("#"):
                    if fields[1] != gain:
                        rows.append((fields[1], fields[0], line.strip()))

    rows += [(gain, channel, fit_row(channel, gain, fitted))
...
def main():
    ...
    parser.add_argument("-H", "--keep-heater-on", action="store_true",
                        help="keep the samples taken while the heater was on")
    ...
    # what went into the fit, so the table can be read without the command line
    # that made it:
    left_out = [name for name, kept in
                [("heater-on", args.keep_heater_on),
    ...
    fitfile = write_fits(args.fit_file, args.gain, fits, notes)
